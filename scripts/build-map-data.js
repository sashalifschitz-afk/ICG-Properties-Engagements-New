...
const VALID_LAYERS = ['active-sale', 'active-lease', 'closed-sale',
                      'closed-lease', 'tenant-rep', 'buyer-rep', 'office'];
...
// The map renders just three pin layers — "sales", "leases", "office".
// Tables are still mapped to the six detailed categories via AIRTABLE_LAYER_MAP
// (so popups can keep showing the specific label, e.g. "Sold"), but each pin's
// LAYER is collapsed to one of these on output so the legend has only Sales / Leases.
const COLLAPSE_LAYER = {
  'active-sale':  'sales',
  'closed-sale':  'sales',
  'buyer-rep':    'sales',
  'active-lease': 'leases',
...
// Office region membership for the three nested Company map layers.
// Always "nationwide"; "texas" if the address is in TX; "austin" if the city is Austin.
function officeRegions(r) {
  const addr = (val(r, null, ['Street Address', 'Address', 'Street', 'Full Address']) || '') + '';
  const regions = ['nationwide'];
  if (/,\s*TX\b/i.test(addr)) regions.push('texas');
  if (/,\s*Austin,\s*TX\b/i.test(addr)) regions.push('austin');
  return regions;
}
...
async function run() {
  ...
  for (const t of sources) {
    ...
    records.forEach(r => {
      ...
      pins.push({
        id:       r.id,
        layer:    COLLAPSE_LAYER[t.layer] || t.layer,
        label:    t.label,
        lat, lng,
        ...
        image:    imageUrl(r),
        meta:     meta(r),
        // Office pins carry nested region membership (Austin ⊂ Texas ⊂ Nationwide),
        // derived from the street address, so map.html can offer the three Company
        // toggles while still rendering a single de-duped pin per office.
        ...(t.layer === 'office' ? { regions: officeRegions(r) } : {}),
      });
      added++;
    });
